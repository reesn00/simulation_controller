...
新架构 ``simulation server → gdr → etl`` 下, task 可能死在任意阶段:
    - 死在 gdr 阶段: 仅有 ``gdr_refined_path`` (C2 单文件) 可能写了一半或缺失
    - 死在 etl 阶段: ``gdr_refined_path`` + ``etl_*_path`` 都可能存在/部分

职责:
    1. 读 SQLite ``tasks WHERE state='dead'``
    2. 把 ``src_path`` 与 ``gdr_refined_path`` / ``etl_*_path``（存在的）
       move 到 ``dead_dir/<batch>_<task_id>__<src_basename>``
    3. 写 ``dead.log`` 与 ``dead/INDEX.jsonl``
    4. 标记 task 已经归档: dead_count 在 batches 表里 ++; task 本身的 state
       仍保持 ``dead`` 不变 (worker 不会再去拉)。
...
def reap_dead(
    ...
    *,
    ...
    Returns: 被归档的 ``DeadArchive`` 列表（一个 task 可能移 0~N 个文件）。
    """
