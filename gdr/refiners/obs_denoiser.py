...
def refine(block: ToolresultBlock, context: dict, defects: list[str], cfg) -> str | None:
    ...
    system_prompt = load_and_render("obs", "system")
    ...
    for attempt in range(cfg.max_retries_9b):
        try:
            ...
            last_error = str(e)
            log.debug("obs_denoiser attempt %d failed: %s; raw=%r", attempt + 1, e, text[:400] if isinstance(text, str) else text)
            continue
