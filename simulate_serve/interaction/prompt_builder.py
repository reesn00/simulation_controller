...
def build_followup_prompt(context: InteractionContext, report: ValidationReport) -> str:
    ...
    gaps = "\n".join(
        f"- 缺口：{item.message}\n  建议表达：{task.interaction_policy.guidance_by_reason.get(item.reason_code) or criteria[item.criterion_id].remediation.guidance}"
        for item in failed
...
{no_leak_rules}请以当前用户身份自然追问，只针对以上可修复缺口；要求对方保留已经满足的内容，并返回一份包含全部要求的完整的修订结果。"""
